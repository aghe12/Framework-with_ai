...
export default class AuthPage {
  ...
  private Elements = {
    ...
    loginButton: '#customer_login input[type="submit"]',
  ...
  private async handleCaptcha(error: any, message: string) {
    ...
    if (url.includes("/challenge") || (await captcha.count()) > 0) {
      console.log(`⚠️ CAPTCHA/BOT DETECTION triggered. ${message}`);
      return;
    }
  ...
  async verifyLoginSuccess() {
    ...
    try {
      ...
      await this.handleCaptcha(
        error,
        "Cannot verify automated login success without solving.",
      );
    }
  }
  ...
  async verifyErrorMessage(expectedMessage: string) {
    try {
      ...
      const errorText = await this.page
        .locator(this.Elements.errorLocator)
        .textContent();
      expect(errorText).toContain(expectedMessage);
    } catch (error) {
      await this.handleCaptcha(
        error,
        "Cannot verify automated error message without solving.",
      );
    }
  }
}
