...
BeforeAll(async function () {
  ...
  const csvFilePath = path.join(
    __dirname,
    "../helper/util/test-data/testData.csv",
  );
  if (fs.existsSync(csvFilePath)) {
    allData = await CSVParser.parseData(csvFilePath, "|");
  } else {
    console.log("⚠️ No CSV test data found, proceeding without it.");
  }
});
  ...
  if (key) {
    ...
    if (testData.Group) {
      try {
        ...
        fixture.logger?.error(
          `Failed to parse Group field as JSON: ${testData.Group}`,
        );
      }
    }
    ...
    pickle.steps.forEach((step, index) => {
      pickle.steps[index].text = step.text.replace(/<([\w.]+)>/g, (_, key) => {
        ...
        let resolvedValue = Array.isArray(value) ? value[0] : value;
    ...
    this.stepArgsMap = new Proxy(
      ...
      {
        get: (_, prop: string) =>
          testData[prop] ?? testData[prop.toLowerCase()] ?? `<${prop}>`,
      },
    );
...
After(async function ({ pickle, result }) {
  ...
  if (result?.status === Status.PASSED) {
    ...
    await this.attach(
      `<a href="https://trace.playwright.dev/" target="_blank">Open Trace</a>`,
      "text/html",
    );
